...
---@param nodes TreemonkeyNode[]
---@param include_root boolean?
---@param is_root fun(node: TreemonkeyNode): boolean
---@return integer, integer
local function iterate(nodes, include_root, is_root)
	local N = #nodes
	...
	local nodeN = nodes[N]
	local end_ = (not include_root and is_root(nodeN)) and (N - 1) or N
...
local function choose_node(nodes, opts, is_root)
	if #nodes == 0 then
		return
	end
	...
	local start, end_ = iterate(nodes, opts.include_root, is_root)
...
--- Gather candidate nodes, present labels, and return the chosen node.
---
--- This is the backend-agnostic engine. A backend supplies a `gather`
--- function that returns candidate |TreemonkeyNode|s ordered from the
--- innermost to the outermost, and an `is_root` predicate used to decide
--- whether the outermost node should be excluded from the choice.
---@param gather fun(opts: TreemonkeyOpts): TreemonkeyNode[]
---@param is_root fun(node: TreemonkeyNode): boolean
---@param opts TreemonkeyOpts?
---@return TreemonkeyItem?
function M.get(gather, is_root, opts)
	opts = init_opts(opts)
	local nodes = gather(opts)
	local item = choose_node(opts.filter and opts.filter(nodes) or nodes, opts, is_root)
	M.clear()
	vim.cmd.redraw()
	return item
end

--- Run a backend's `get`, then apply the action to the chosen item.
---@param get fun(opts: TreemonkeyOpts?): TreemonkeyItem?
---@param opts TreemonkeyOpts?
---@return nil
function M.select(get, opts)
	local ok, result = pcall(get, opts)
	if ok then
		if result then
